...
export function InvestorSignUpForm({ onBack, onSuccess }: InvestorSignUpFormProps) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [firstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [investorType, setInvestorType] = useState<"basic" | "qualified">("basic");
    ...
    <>
      ...
      <Card className="w-[350px]">
        <CardHeader>
          ...
          <CardTitle>التسجيل كمستثمر</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Select
              value={investorType}
              onValueChange={(value: "basic" | "qualified") => setInvestorType(value)}
            >
              <SelectTrigger>
